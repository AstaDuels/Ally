--[[
    ASTA REMAKE – Versão final com Drop Brainrot corrigido
]]
...
local function startSpeedBoosterLoop()
    ...
    speedBoosterConn = RunService.Heartbeat:Connect(function()
        if not speedBoosterActive or not carrySpeedActive then return end
        ...
        if moveDir.Magnitude > 0.1 then
            ...
            linearVelocity.PlaneVelocity = Vector2.new(flat.X * CS, flat.Z * CS)
        else
            linearVelocity.PlaneVelocity = Vector2.zero
        end
    end)
...
local function resetCharacterRotation()
    ...
    chatMessage("🔄 Rotação do boneco corrigida!")
end

-- ============================================================
-- DROP BRAINROT CORRIGIDO (funciona de verdade)
-- ============================================================
...
local function runDrop()
    ...
    -- Desativar modos que interferem
    ...
    dropActive = true
    chatMessage("💥 Drop Brainrot ATIVADO!")

    -- Salvar velocidade original
    local originalVel = root.AssemblyLinearVelocity

    -- Aplicar fling com BodyVelocity (mais estável)
    ...
    -- Também aplicar velocidade diretamente para garantir
    ...
    task.delay(0.8, function()
        pcall(function()
            ...
            if root and root.Parent then
                root.AssemblyLinearVelocity = Vector3.new(originalVel.X, math.max(originalVel.Y, 0), originalVel.Z)
            end
        end)
        dropActive = false
        chatMessage("💥 Drop Brainrot finalizado!")
    end)

    -- Colisão com outros jogadores (opcional, mas mantido do original)
    task.spawn(function()
            ...
            if p ~= LP and p.Character then
                for _,part in ipairs(p.Character:GetDescendants()) do
                    if part:IsA("BasePart") then
                        ...
                        task.delay(0.8, function() pcall(function() part.CanCollide = true end) end)
                    end
                end
            end
        end
...
local ASTA={medusaReset=false,antiKick=false,brainrot=false,tpBat=false,batV2=false,tpConn=nil,v2Conn=nil,v2Rot=nil,hitCD=false,v2CD=false}
...
local function saveBtnPositions() end
...
local function buildMobileButtons()
    ...
    local cols = 3
    local spacing = 16
    local totalWidth = cols * (BTN_SIZE + spacing) - spacing

    local startX = -totalWidth - 20
    local startY = 60
    ...
    local buttonOrder = {
        ...
        {key="autoBat",    label="BAT AIMBOT",  toggle=true},
        {key="tpBat",      label="TP BAT",      toggle=true},
    ...
    local function makeButton(def, index)
        ...
        local xPos = startX + col * (BTN_SIZE + spacing)
        local yPos = startY + row * (BTN_SIZE + spacing)
        ...
        frame.Position = UDim2.new(1, initX, 0, initY)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if def.key == "autoLeft" then
                ...
                if autoLeftEnabled then
                    ...
                    chatMessage("🔁 Auto Left ATIVADO")
                else
                    stopAutoLeft()
                    if autoLeftSetVisual then autoLeftSetVisual(false) end
                    chatMessage("🔁 Auto Left DESATIVADO")
                end
            ...
            if def.key == "autoRight" then
                ...
                if autoRightEnabled then
                    ...
                    chatMessage("🔁 Auto Right ATIVADO")
                else
                    stopAutoRight()
                    if autoRightSetVisual then autoRightSetVisual(false) end
                    chatMessage("🔁 Auto Right DESATIVADO")
                end
            ...
            if def.key == "autoBat" then
                if autoBatEnabled then
                    ...
                    if autoBatSetVisual then autoBatSetVisual(false) end
                    if mobBtnRefs.autoBat then mobBtnRefs.autoBat(false) end
                    chatMessage("⚔️ Bat Aimbot DESATIVADO")
                else
                    if autoLeftEnabled then autoLeftEnabled=false; stopAutoLeft(); if autoLeftSetVisual then autoLeftSetVisual(false) end; if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end end
                    if autoRightEnabled then autoRightEnabled=false; stopAutoRight(); if autoRightSetVisual then autoRightSetVisual(false) end; if mobBtnRefs.autoRight then mobBtnRefs.autoRight(false) end end
                    queueAutoBatStart()
                    ...
                    chatMessage("⚔️ Bat Aimbot ATIVADO")
                end
            ...
            if def.key == "tpBat" then
                if ASTA.tpBat then
                    ASTA.stopTPBat()
                    if ASTA.setTPBatVisual then ASTA.setTPBatVisual(false) end
                    chatMessage("🌀 TP Bat DESATIVADO")
                else
                    ASTA.startTPBat()
                    if ASTA.setTPBatVisual then ASTA.setTPBatVisual(true) end
                    chatMessage("🌀 TP Bat ATIVADO")
                end
                ASTA.tpBat = not ASTA.tpBat
                setOn(ASTA.tpBat)
                if mobBtnRefs.tpBat then mobBtnRefs.tpBat(ASTA.tpBat) end
                saveConfig(); return
            end
            if def.key == "batV2" then
                if ASTA.batV2 then
                    ASTA.stopBatV2()
                    if ASTA.setBatV2Visual then ASTA.setBatV2Visual(false) end
                    chatMessage("🔥 Bat V2 DESATIVADO")
                else
                    ASTA.startBatV2()
                    if ASTA.setBatV2Visual then ASTA.setBatV2Visual(true) end
                    chatMessage("🔥 Bat V2 ATIVADO")
                end
                ASTA.batV2 = not ASTA.batV2
                setOn(ASTA.batV2)
                if mobBtnRefs.batV2 then mobBtnRefs.batV2(ASTA.batV2) end
                saveConfig(); return
            end
...
LP.CharacterAdded:Connect(function(char)
    ...
    if carrySpeedActive then task.wait(0.2);toggleSpeedBooster() end
    task.wait(0.2)
    local hum = char:FindFirstChildOfClass("Humanoid")
    if hum then
        hum.AutoRotate = true
...
local function onAnchorChanged(part)
    return part:GetPropertyChangedSignal("Anchored"):Connect(function()
        if part.Anchored and part.Transparency==1 then
            if ASTA.medusaReset then cursedInstaReset()
...
function ASTA.disableAntiKick() ASTA.antiKick=false;ASTA.brainrot=false end
...
queueAutoBatStart=function()
    if ASTA.antiKick and ASTA.brainrot then return end
    if autoLeftEnabled then autoLeftEnabled=false;if autoLeftSetVisual then autoLeftSetVisual(false) end;stopAutoLeft() end
    if autoRightEnabled then autoRightEnabled=false;if autoRightSetVisual then autoRightSetVisual(false) end;stopAutoRight() end
    startBatAimbot()
end
...
local function resetAllSettings()
    ...
    ASTA.medusaReset=false;ASTA.disableAntiKick()
    ...
    if carrySpeedActive then toggleSpeedBooster() end
    if refreshSpeedModeLabel then refreshSpeedModeLabel() end
    if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(carrySpeedActive) end
    if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerModeEnabled) end
    if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end
...
local function speedToggleAction() toggleCarryMode(); saveConfig() end
...
-- TP BAT + BAT V2
-- ============================================================
local ASTA_SLAP_LIST={"Bat","Slap","Iron Slap","Gold Slap","Diamond Slap","Emerald Slap","Ruby Slap","Dark Matter Slap","Flame Slap","Nuclear Slap","Galaxy Slap","Glitched Slap"}
function ASTA.findBat()
    local char=LP.Character; if not char then return nil end
    for _,name in ipairs(ASTA_SLAP_LIST) do local t=char:FindFirstChild(name); if t and t:IsA("Tool") then return t end end
    local bp=LP:FindFirstChildOfClass("Backpack")
    if bp then
        for _,name in ipairs(ASTA_SLAP_LIST) do
...
function ASTA.closestRoot()
    ...
    if best then
        ...
        chatMessage("⚠️ Nenhum jogador próximo para este modo de Bat.")
        return nil,math.huge
    end
end
function ASTA.tpHit()
    if ASTA.hitCD then return end
    ASTA.hitCD=true
    pcall(function()
        local bat=ASTA.findBat()
        if bat then
            bat:Activate()
    ...
    task.delay(0.08,function() ASTA.hitCD=false end)
end
function ASTA.startTPBat()
    if ASTA.tpConn then return end
    ASTA.tpBat=true
    ASTA.tpConn=RunService.Heartbeat:Connect(function()
        if not ASTA.tpBat then return end
        local char=LP.Character; if not char then return end
        local hrp=char:FindFirstChild("HumanoidRootPart"); if not hrp then return end
        local tr=ASTA.closestRoot()
        if tr then
...
function ASTA.stopTPBat()
    if ASTA.tpConn then ASTA.tpConn:Disconnect();ASTA.tpConn=nil end
    ASTA.tpBat=false
end
function ASTA.v2Swing()
    if ASTA.v2CD then return end
    ASTA.v2CD=true
    pcall(function()
        local char=LP.Character; if not char then return end
        local bat=ASTA.findBat()
    ...
    task.delay(0.35,function() ASTA.v2CD=false end)
end
function ASTA.startBatV2()
    if ASTA.v2Conn then ASTA.v2Conn:Disconnect() end
    ASTA.batV2=true
    local hum0=LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
    if hum0 then
        if ASTA.v2Rot==nil then ASTA.v2Rot=hum0.AutoRotate end
        hum0.AutoRotate=false
    end
    ASTA.v2Conn=RunService.RenderStepped:Connect(function()
        if not ASTA.batV2 then return end
        local char=LP.Character; if not char then return end
        ...
        local target,targetDist=ASTA.closestRoot()
        if not target then return end
        local myPos=root.Position
        local targetPos=target.Position
        ...
        if targetDist<=8 then ASTA.v2Swing() end
    end)
end
function ASTA.stopBatV2()
    ASTA.batV2=false
    if ASTA.v2Conn then ASTA.v2Conn:Disconnect();ASTA.v2Conn=nil end
    ASTA.v2CD=false
    ...
    if hum then
        hum.AutoRotate=(ASTA.v2Rot==nil) and true or ASTA.v2Rot
        hum.PlatformStand=false
        pcall(function() hum:ChangeState(Enum.HumanoidStateType.GettingUp) end)
    end
    ASTA.v2Rot=nil
...
pcall(function()
    ...
    if type(d.medusaReset)=="boolean" then ASTA.medusaReset=d.medusaReset end
...
pcall(function()
    ...
    if ASTA.medusaReset then task.spawn(function() task.wait(1); local ch=LP.Character; if ch and setupMedusa then setupMedusa(ch) end end) end
    ...
    if carrySpeedActive then task.spawn(function() task.wait(0.8); toggleSpeedBooster() end) end
end)
...
;(function()
    ...
    local _,svMedReset=addToggleRow(cp,"Medusa Reset",ASTA.medusaReset,8,nil,function(on)
        ASTA.medusaReset=on
        if on then setupMedusa(LP.Character) elseif not medusaCounterEnabled then stopMedusaCounter() end
        saveConfig()
    end)
    ASTA.setMedusaResetVisual=svMedReset
    addSectLbl(cp,"PROTECTION",13)
    local _,svAntiKick=addToggleRow(cp,"Anti Kick",ASTA.antiKick,14,nil,function(on)
    ...
    local _,svTPBat=addToggleRow(cp,"TP Bat",ASTA.tpBat,16,nil,function(on)
        if on then ASTA.startTPBat() else ASTA.stopTPBat() end
        if mobBtnRefs.tpBat then mobBtnRefs.tpBat(on) end
    end)
    ASTA.setTPBatVisual=svTPBat
    local _,svBatV2=addToggleRow(cp,"Bat V2",ASTA.batV2,17,nil,function(on)
        if on then ASTA.startBatV2() else ASTA.stopBatV2() end
        if mobBtnRefs.batV2 then mobBtnRefs.batV2(on) end
    end)
    ASTA.setBatV2Visual=svBatV2
...
;(function()
    ...
    addSectLbl(vi,"VISUAL EFFECTS",0)
    ...
    resetBtn.BackgroundColor3 = Color3.fromRGB(150,30,40)
    ...
    resetBtn.MouseButton1Click:Connect(function()
        ...
        chatMessage("🔄 Rotação do boneco corrigida!")
        tw(resetBtn, {BackgroundColor3 = C.white})
        task.delay(0.15, function() tw(resetBtn, {BackgroundColor3 = Color3.fromRGB(150,30,40)}) end)
    end)
    local hovReset = Instance.new("TextButton", resetRow)
    hovReset.Size = UDim2.new(1,0,1,0)
    hovReset.BackgroundTransparency = 1
    hovReset.Text = ""
    hovReset.ZIndex = 0
    hovReset.MouseEnter:Connect(function() tw(resetRow,{BackgroundTransparency=0.3}) end)
    hovReset.MouseLeave:Connect(function() tw(resetRow,{BackgroundTransparency=0.5}) end)

    addSectLbl(vi,"SKY THEME",8)
    local skyIdx=1; for i,t in ipairs(SkyOrder) do if t==currentSkyTheme then skyIdx=i;break end end
    local skyRow=Instance.new("Frame",vi)
    skyRow.Size=UDim2.new(1,0,0,38); skyRow.BackgroundColor3=C.row
    ...
    local fovRow=Instance.new("Frame",vi); fovRow.Size=UDim2.new(1,0,0,38); fovRow.BackgroundColor3=C.row
    ...
    addSectLbl(vi,"RESET ALL",14)
    local resetRow2=Instance.new("Frame",vi); resetRow2.Size=UDim2.new(1,0,0,38); resetRow2.BackgroundColor3=C.row
    resetRow2.BackgroundTransparency=0.5; resetRow2.BorderSizePixel=0; resetRow2.LayoutOrder=15; resetRow2.Parent=vi
    guiCorner(resetRow2,10); guiStroke(resetRow2,C.divider,1)
    local resetLbl2=Instance.new("TextLabel",resetRow2); resetLbl2.Size=UDim2.new(0.55,0,0,16); resetLbl2.Position=UDim2.new(0,12,0,6)
    resetLbl2.BackgroundTransparency=1; resetLbl2.Text="Reset All Settings"; resetLbl2.TextColor3=C.text; resetLbl2.TextSize=11; resetLbl2.Font=Enum.Font.GothamBold; resetLbl2.TextXAlignment=Enum.TextXAlignment.Left
    local resetBtn2=Instance.new("TextButton",resetRow2); resetBtn2.Size=UDim2.new(0,52,0,22); resetBtn2.Position=UDim2.new(1,-60,0.5,-11)
    ...
    local hov4=Instance.new("TextButton",resetRow2); hov4.Size=UDim2.new(1,0,1,0); hov4.BackgroundTransparency=1; hov4.Text=""; hov4.ZIndex=0
    hov4.MouseEnter:Connect(function() tw(resetRow2,{BackgroundTransparency=0.3}) end); hov4.MouseLeave:Connect(function() tw(resetRow2,{BackgroundTransparency=0.5}) end)
end)()
...
if ASTA.medusaReset then setupMedusa(LP.Character) end
...
print("ASTA REMAKE LOADED – Drop Brainrot corrigido!")
